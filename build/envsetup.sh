# Aospify functions that extend build/envsetup.sh
function __print_aosp_functions_help() {
cat <<EOF
Additional Aospify functions:
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Aospify model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $AOSPIFY_FIXUP_COMMON_OUT ]; then
